 ...
 *			</ul>
 *
 * @note	If you want to purchase Foxit PDF SDK license and use ANY of the following functions, please request for enabling PDFSecurity module explicitly.
 */
...
typedef struct _FSPDF_SECURITYHANDLER
{
	 ...
	 * @param[in]	encryptInfo		Pointer to a ::FSCRT_BSTR structure to specify encryption info data.
	 ...
	 *									If it is <b>NULL</b>, parameter <i>keyLen</i> should not be <b>NULL</b> and receive the required length of key buffer.
	 * @param[in,out]	keyLen			Pointer to a ::FS_DWORD object that receives the length (number of bytes) of key buffer, 
	 *									it should not be <b>NULL</b>.
	 *
	 * @return	::FSCRT_ERRCODE_SUCCESS if successful.<br>
	 *			For more error codes, please refer to macro definitions <b>FSCRT_ERRCODE_XXX</b>.
	 *
	 * @note	If standard encryption for encryption and decryption is used,
	 ...
	 *			for the same data stream to be decrypted.
	 */
	 ...
	 *			For more error codes, please refer to macro definitions <b>FSCRT_ERRCODE_XXX</b>.
	 *
	 * @note	It's time to free the internal handler structure.
	 */
	 ...
	 * @brief	Encrypt data.
	 *
	 * @details	Final encrypted data block should be output in parameter <i>dstLen</i>.
	 *
	 * @param[in]		clientData		Pointer to the user-supplied data.
	 * @param[in]		context			Pointer to context handle which is created by callback function FSPDF_SECURITYHANDLER::CreateContext.
...
typedef struct _FSPDF_SECURITYHANDLER_CERT
{
	 ...
	 *			first to receive the required length of the source seed buffer, and second to receive the source seed buffer.
	 *
 ...
 *
 * @details	This function is only used for custom encryption and decryption.<br>
 *			If user wants to use custom security handler, this function must be called first before doing customer encryption or decryption.<br>
 *			Once no more custom encryption or decryption is needed, function ::FSPDF_Security_UnregisterHandler shoulde be called to unregister current customer security handler.
 *
